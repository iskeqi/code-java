...
import com.keqi.projectseedfirst.common.AjaxEntityBuilder;
...
public class SecurityInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws IOException {
		...
		// 1、调用SSO进行登录认证
		LoginUserBO loginUserBO = this.authentication(request.getHeader(CommonEnum.ACCESS_TOKEN.name()));
		if (loginUserBO == null) {
		...
		request.setAttribute(CommonEnum.LOGIN_USER.name(), loginUserBO);
		return true;
	}
